...
def test_recorded_checkpoint_contract_validates_hindi_text_conditioning():
    model = _model(self_qk_norm=True, cross_qk_norm=True)
    model.text_encoder = SimpleNamespace(
        model_name="models/muril-base-cased",
        model_revision="a" * 40,
        max_length=50,
        output_dim=768,
    ...
    checkpoint = {MODEL_ARCHITECTURE_KEY: build_model_architecture_contract(model)}
    contract = validate_checkpoint_architecture_contract(checkpoint, model)
    assert contract["text_conditioning"]["adapter"] == {
        "type": "tokenwise_residual_mlp",
        "hidden_dim": 3072,
    }


def test_recorded_checkpoint_contract_rejects_hindi_text_length_mismatch():
    trained = _model(self_qk_norm=True, cross_qk_norm=True)
    trained.text_encoder = SimpleNamespace(
        model_name="models/muril-base-cased",
        model_revision="a" * 40,
        max_length=50,
        output_dim=768,
    ...
    current = _model(self_qk_norm=True, cross_qk_norm=True)
    current.text_encoder = SimpleNamespace(
        model_name="/different/machine/path/muril-base-cased",
        model_revision="a" * 40,
        max_length=96,
        output_dim=768,
        adapter_hidden_dim=3072,
    ...
    checkpoint = {MODEL_ARCHITECTURE_KEY: build_model_architecture_contract(trained)}
    with pytest.raises(RuntimeError, match="text conditioning"):
        validate_checkpoint_architecture_contract(checkpoint, current)
