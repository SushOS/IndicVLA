...
def test_experiment_presets_share_omnimodal_sinusoidal_training_protocol(
    ...
    assert list(cfg.init_weights_ignore_prefixes) == []
    assert cfg.init_weights_legacy_attention_contract is None
...
def test_train_config_accepts_weights_only_ignore_prefixes():
    with initialize_config_dir(config_dir=str(GENERATION_CONFIG_DIR), version_base=None):
        cfg = compose(
            config_name="train",
            overrides=["init_weights_ignore_prefixes=[text_encoder.]"],
        )
