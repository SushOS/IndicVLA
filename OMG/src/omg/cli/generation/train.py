...
def _load_weights_only_checkpoint(
    ...
    *,
    ...
    if isinstance(ignore_prefixes, str):
        raise TypeError("init_weights_ignore_prefixes must be a collection of prefixes, not a string")
    normalized_ignore_prefixes = tuple(dict.fromkeys(ignore_prefixes or ()))
    if any(not isinstance(prefix, str) or not prefix for prefix in normalized_ignore_prefixes):
        raise ValueError("init_weights_ignore_prefixes entries must be non-empty strings")
    if MODEL_ARCHITECTURE_KEY in checkpoint or normalized_ignore_prefixes:
        ignores_text_encoder = any(
            "text_encoder.".startswith(prefix)
            for prefix in normalized_ignore_prefixes
        )
        validate_checkpoint_architecture_contract(
            checkpoint,
            model,
            legacy_attention_contract=legacy_attention_contract,
            validate_text_conditioning=not ignores_text_encoder,
        )
    state_dict = checkpoint["state_dict"]

    if normalized_ignore_prefixes:
        def is_ignored(key: str) -> bool:
            return key.startswith(normalized_ignore_prefixes)

        current_state = model.state_dict()
        checkpoint_keys = {key for key in state_dict if not is_ignored(key)}
        model_keys = {key for key in current_state if not is_ignored(key)}
        missing = sorted(model_keys - checkpoint_keys)
        unexpected = sorted(checkpoint_keys - model_keys)
        mismatched = []
        for key in sorted(checkpoint_keys & model_keys):
            checkpoint_shape = getattr(state_dict[key], "shape", None)
            model_shape = getattr(current_state[key], "shape", None)
            normalized_checkpoint_shape = tuple(checkpoint_shape) if checkpoint_shape is not None else None
            normalized_model_shape = tuple(model_shape) if model_shape is not None else None
            if normalized_checkpoint_shape != normalized_model_shape:
                mismatched.append(
                    f"{key}: checkpoint={normalized_checkpoint_shape} model={normalized_model_shape}"
                )
        if missing or unexpected or mismatched:
            raise RuntimeError(
                f"Failed to load weights-only checkpoint from {path} with ignored prefixes "
                f"{list(normalized_ignore_prefixes)}: missing_non_ignored={missing}, "
                f"unexpected_non_ignored={unexpected}, mismatched_non_ignored={mismatched}"
            )

        # Retain the model's initialized values for ignored subtrees and replace every
        # non-ignored value only after the complete key/shape contract has been checked.
        for key in checkpoint_keys:
            current_state[key] = state_dict[key]
        model.load_state_dict(current_state, strict=True)
        ignored_checkpoint_keys = sorted(key for key in state_dict if is_ignored(key))
        ignored_model_keys = sorted(key for key in current_state if is_ignored(key))
        Log.info(
            "[Checkpoint]: initialized model weights strictly from %s while ignoring prefixes=%s "
            "ignored_checkpoint_keys=%s retained_model_keys=%s",
            path,
            list(normalized_ignore_prefixes),
...
def run(cfg: DictConfig) -> None:
    ...
    if init_weights_only_ckpt:
        ...
        _load_weights_only_checkpoint(
            ...
            ignore_prefixes=cfg.get("init_weights_ignore_prefixes"),
            legacy_attention_contract=cfg.get("init_weights_legacy_attention_contract"),
        )
